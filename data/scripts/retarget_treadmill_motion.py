...
def retarget_motion_with_mink(
    ...
    # Prepare arrays for SkeletonMotion
    root_translations = np.array(all_root_translations)  # shape (n_frames, 3)
    root_rotations_wxyz = np.array(all_root_rotations)  # shape (n_frames, 4)
    # Joint angles (Euler) to quaternions
    rotations_xyz = np.array(all_joint_angles)  # shape (n_frames, n_joints * 3)
    n_joints = len(model_joint_names) - 1  # Exclude root
    joint_quat = np.array(euler_to_quaternion(rotations_xyz)).reshape(n_frames, n_joints, 4)

    # Build global/local rotations arrays
    # Root rotation is first joint, then all others
    global_rotations = np.zeros((n_frames, n_joints + 1, 4))
    local_rotations = np.zeros((n_frames, n_joints + 1, 4))
    global_rotations[:, 0, :] = root_rotations_wxyz  # root
    global_rotations[:, 1:, :] = joint_quat  # joints
    local_rotations[:, 0, :] = root_rotations_wxyz  # root
    local_rotations[:, 1:, :] = joint_quat  # joints

    # Prepare global translations
    global_translations = np.zeros((n_frames, n_joints + 1, 3))
    global_translations[:, :] = global_translations  # already filled above

    # Create SkeletonMotion
    sk_motion = SkeletonMotion(
        global_translation=global_translations,
        global_rotation=global_rotations,
        local_rotation=local_rotations,
        root_translation=root_translations,
        root_rotation=root_rotations_wxyz,
        skeleton_tree=skeleton_tree,
        fps=fps,
        is_local=True
    )

    print(f"[DEBUG] SkeletonMotion shapes:")
    print(f"  global_translation: {sk_motion.global_translation.shape}")
    print(f"  global_rotation: {sk_motion.global_rotation.shape}")
    print(f"  local_rotation: {sk_motion.local_rotation.shape}")
    print(f"  root_translation: {sk_motion.root_translation.shape}")
    print(f"  root_rotation: {sk_motion.root_rotation.shape}")
