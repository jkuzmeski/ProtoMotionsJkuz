...
def main(
        motion_file: Path,
        outpath: Path,
        retargeted_data_path: Path = None,
    ...
    for motion_file, outpath in motion_files:
        ...
        # Edit file paths - since we now use absolute paths in YAML, no modification needed
        # The motion files already have the correct absolute paths
        for motion in motion_data["motions"]:
            # Check if the file path is already absolute
            if not os.path.isabs(motion["file"]):
                # If retargeted_data_path is provided, use it as the base path
                # Otherwise, assume the files are relative to the motion file location
                if retargeted_data_path is not None:
                    motion["file"] = str(retargeted_data_path.resolve() / motion["file"])
                else:
                    # Assume files are in the same directory as the motion file or use relative paths
                    motion_file_dir = Path(motion_file).parent
                    motion["file"] = str(motion_file_dir.resolve() / motion["file"])
